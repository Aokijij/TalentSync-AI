...
def test_authentication_and_permissions(client):
    ...
    response = client.post(
        "/api/v1/auth/login",
        json={"email": user["email"], "password": "Test-password-2026!"},
    )
...
def test_public_activity_stats_are_available_before_login(client):
    ...
    job = create_job(client, owner)
    client.post(
        "/api/v1/applications", headers=candidate_headers, json={"job_id": job["id"]}
    )
...
def test_screening_questions_adjust_only_the_company_score(client):
    ...
    assert response.status_code == 201, response.text
    job = response.json()
    public_question = client.get(f"/api/v1/jobs/{job['id']}").json()[
        "application_questions"
    ][0]
    ...
    base = client.post(
        f"/api/v1/jobs/{job['id']}/match", headers=candidate_headers
    ...
    company_view = client.get(
        f"/api/v1/applications/jobs/{job['id']}", headers=owner
    ).json()[0]
    assert company_view["screening_adjustment"] == 5
    assert company_view["base_match_percentage"] == pytest.approx(base)
    assert company_view["adjusted_match_percentage"] == pytest.approx(
        min(100, base + 5)
    )
...
def test_notifications_can_be_filtered_and_removed(client):
    ...
    items = client.get(
        "/api/v1/notifications?category=applications&unread_only=false", headers=owner
    ).json()
    assert len(items) == 1
    assert items[0]["category"] == "applications"
    assert (
        client.delete(
            f"/api/v1/notifications/{items[0]['id']}", headers=candidate_headers
        ).status_code
        == 404
    )
    assert (
        client.delete(
            f"/api/v1/notifications/{items[0]['id']}", headers=owner
        ).status_code
        == 204
    )
    assert (
        client.get("/api/v1/notifications?unread_only=false", headers=owner).json()
        == []
    )
...
def test_reopening_covered_vacancy_preserves_history_and_notifications(client, mode):
    ...
    edited = client.put(
        ...
        json={
            ...
            "recruiter_notes": "Seguimiento que debe conservarse",
        },
    )
    ...
    notifications = client.get(
        "/api/v1/notifications?unread_only=false", headers=candidate_headers
    ).json()
    ...
    assert (
        client.patch(
            f"/api/v1/jobs/{job['id']}", headers=owner, json={"status": "active"}
        ).status_code
        == 422
    )
    ...
    assert (
        client.get("/api/v1/applications/me", headers=candidate_headers).json()
        == before
    )
    assert (
        client.get(
    ...
    assert (
        client.get("/api/v1/applications/me", headers=second_headers).json()
        == other_before
    )
    assert (
        client.get(
    ...
    if mode == "continue":
        ...
        moved = client.put(
            f"/api/v1/applications/{other_application['id']}/status",
            headers=owner,
            json={"pipeline_stage": "reviewing"},
        )
        ...
        assert (
            client.get(
                f"/api/v1/applications/jobs/{reopened['id']}", headers=owner
            ).json()
            == []
        )
...
def test_language_levels_are_saved_and_company_candidate_scores_are_identical(client):
    ...
    updated = client.patch(
        ...
        headers=owner,
        json={"languages": [{"name": "English", "level": "b2"}]},
    )
    assert updated.status_code == 200, updated.text
    ...
    assert (
        client.get("/api/v1/profiles/me", headers=headers).json()["languages"]
        == saved.json()["languages"]
    )
    ...
    assert (
        next(item for item in recommendations if item["job_id"] == job["id"])[
            "match_percentage"
        ]
        == match["match_percentage"]
    )
    invitation = client.post(
        f"/api/v1/recommendations/jobs/{job['id']}/candidates/{candidate['id']}/invite",
        headers=owner,
    )
    ...
    assert (
        client.put("/api/v1/profiles/me", headers=headers, json=profile).status_code
        == 422
    )
    profile["languages"] = [
...
def test_talent_search_and_invitations_start_at_fifty_percent(
    client, monkeypatch, score, expected_count, invite_status
):
    from app.application.use_cases import recommendations

    _, owner = company_account(client, "empresa-cincuenta")
    candidate, _ = register(
        client, "candidato-cincuenta", profession="Analista", skills=["SQL"]
    )
    job = create_job(client, owner)
    monkeypatch.setattr(
        recommendations,
        "rank_candidates_for_job",
        lambda db, job, *, nlp: [(db.users.get(candidate["id"]), score)],
    )
    monkeypatch.setattr(
        recommendations,
        "upsert_recommendation",
        lambda db, candidate, job, *, nlp: SimpleNamespace(match_percentage=score),
    )
...
def test_company_candidate_page_uses_eighty_percent_and_pagination_contract(
    client, monkeypatch
):
    from app.application.use_cases import recommendations
    ...
    first, _ = register(
        client,
        "perfil-destacado",
        profession="Coordinador comercial",
        skills=["Comunicación"],
    )
    second, _ = register(
        client,
        "perfil-regular",
        profession="Asistente comercial",
        skills=["Organización"],
    )
    job = create_job(client, owner)
    monkeypatch.setattr(
...
def test_hiring_keeps_process_open_until_company_marks_job_filled(client):
    ...
    assert (
        client.post(invite_url, headers=owner).json()["id"] == invitation.json()["id"]
    )
    ...
    still_active = client.get("/api/v1/applications/me", headers=second_headers).json()[
        0
    ]
    ...
    finalized = client.get("/api/v1/applications/me", headers=second_headers).json()[0]
...
def test_profile_photo_and_resume_style_are_saved(client, monkeypatch, tmp_path):
    ...
    assert (
        client.get("/api/v1/profiles/me", headers=headers).json()["resume_color"]
        == "teal"
    )
    invalid = client.put(
        "/api/v1/profiles/me",
        headers=headers,
        json={**editable, "resume_color": "not-a-palette"},
    )
    ...
    assert (
        client.get(
            f"/api/v1/profiles/candidates/{user['id']}", headers=company_headers
        ).status_code
        == 403
    )
    application = client.post(
    ...
    client.app.dependency_overrides[get_image_storage] = lambda: SimpleNamespace(
        save=failed_save
    )
    failed = client.post(
        "/api/v1/profiles/me/photo",
        headers=headers,
        files={"file": ("nueva.png", image, "image/png")},
    )
    assert failed.status_code == 503
    client.app.dependency_overrides.pop(get_image_storage)
    assert (
        client.get("/api/v1/profiles/me", headers=headers).json()["photo_url"]
...
def test_admin_syncs_authorized_jooble_catalog(client):
    ...
    status_response = client.get("/api/v1/admin/jobs/sources/jooble", headers=headers)
